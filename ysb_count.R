...
#C:/Users/Jackie/Documents/YSB/
full_file <- read.csv("mergedDataV1.csv")
...
ui <- fluidPage(
  fluidRow(column(width = 10, align = "center", style = "font-size: 20pt; 
                                     line-height: 40pt; 
                  width = 100",
                  tags$strong("YSB - Number of people leaving program subcategory by exit year"))),
...
server <- function(input, output) {
  ...
  output$bubbleplot <- renderPlot({
    p1 <- reactive(subset(full_file, Active.y == 1 & ProgamSubCategory == input$y & exit_year <= input$y2 & exit_year >= input$y1 ) %>% group_by(Program, exit_year)%>%
                     summarise(count = n() )%>%
    ...
    ggplot(data = p1(), aes(x = exit_year, y = Program, 
                            label=count)) + 
      geom_point(aes(size = count*1.5, colour = count),  alpha = 0.7) +
      #labs(title = "Number of people leaving program by exit year") + 
      labs(x = "Exit Year") + 
      labs(y= "Number of people leaving program")  + 
